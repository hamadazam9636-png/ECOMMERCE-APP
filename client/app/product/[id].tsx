...
export default function ProductDetails({route}: any) {
 ...
 const fetchProduct = async () => {
         setProduct(dummyProducts.find((product)=> product._id
        === id) as any)
        setLoading(false)
    }
  ...
  return (
    <View className='flex-1 bg-white'>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        flex-row justify-between items-center z-10'>
           ...
           rounded-full items-center justify-center'>
            ...
            className='w-10 h-10 bg-white/80
           rounded-full items-center justify-center'>
             <Ionicons name={isLiked ? 'heart' :
             'heart-outline'} size={24} color={isLiked ?COLORS.accent : COLORS.primary }/>
           </TouchableOpacity>
        </View>
       ...
       <View className='px-5'>
           ...
           <View className='flex-row justify-between items-start mb-2'>
            <Text className='text-2xl font-bold text-pretty
      ...
      mb-6'>${product.price.toFixed(2)}</Text>
      ...
      {/* Sizes */}
        {product.sizes && product.sizes.length > 0 && (
           ...
           <View className='flex-row gap-3 mb-6 flex-wrap'>
               {product.sizes.map((size)=>(
                   ...
                   justify-center border ${selectedSize === size ?
                    'bg-primary border-primary' : 'bg-white border-gray-100'}`}>
       ...
       {/* Footer */}
       <View className='absolute bottom-0 left-0 flex-row right-0
       p-4 bg-white border-t border-gray-100'>
          <TouchableOpacity onPress={handleAddToCart} className='w-4/5 bg-primary py-4 
          rounded-full items-center shadow-lg flex-row justify-center' >
            <Ionicons name='bag-outline' size={20} color="white"/>
            <Text className='text-white font-bold text-base
            ml-2'>Add to Cart</Text>
          </TouchableOpacity>

         <TouchableOpacity onPress={()=> router.push("/(tabs)/cart")} className='w-1/5 py-3 flex-row justify-center relative' >
            <Ionicons name='cart-outline' size={24} />
            <View className='absolute top-2 right-5 size-4 z-10
            bg-black rounded-full justify-center items-center'>
