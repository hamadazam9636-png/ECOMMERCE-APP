  ...
  return (
    <SafeAreaView className='flex-1 bg-surface'
     edges={['top']}>
      ...
      contentContainerStyle={!user ? {flex:1,
          ...
          {!user ? (
            ...
            <View className='items-center w-full'>
              ...
              bg-gray-200 items-center justify-center mb-6'>
                <Ionicons name='person' size={40} color=
                {COLORS.secondary}/>
              </View>
            <Text className='text-primary font-bold text-xl
            mb-2'>Guest User</Text>
            <Text className='text-secondary text-base mb-8 text-center
            w-3/4 px-4'>Login to view your profile, orders, and 
              addresses.
            </Text>
            <TouchableOpacity onPress={()=>router.push('/sign-in')}
             className='bg-primary w-3/4 py-3 rounded-f.ull
            items-center shadow-lg'>
              <Text className='text-white font-bold 
              text-lg'>Login / Sign Up</Text>
            </TouchableOpacity>
            </View>
          ) : (
            <>
            ...
            border-gray-100/75 p-2 mb-4'>
              {PROFILE_MENU.map((item, index)=>(
                <TouchableOpacity key={item.id}
                className={`flex-row items-center p-4 ${index !== 
                  PROFILE_MENU.length - 1 ? "border-b border-gray-100 "
                   : ""}`}
            ...
            justify-center p-4' onPress={handleLogout}>
