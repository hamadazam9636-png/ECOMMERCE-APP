...
export default function CartItem({item, onRemove, 
    onUpdateQuantity } : CartItemProps) {
  ...
  return (
    <View className='flex-row mb-4 bg-white p-3 rounded-xl'>
      <View className='w-20 h-20 bg-gray-100 rounded-lg
      overflow-hidden mr-3'>
        <Image source={{uri: imageUrl}} className='w-full
        h-full' resizeMode='cover'/>
      </View>
      <View className='flex-1 justify-between'>
        ...
        mt-2'>
            ...
            text-base'>${item.product.price.toFixed(2)}
            </Text>
