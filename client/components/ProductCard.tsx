   ...
   <Link href={`/product/${product._id}`} asChild>
    ...
    rounded-lg overflow-hidden'>
        <View className='relative h-56 w-full bg-gray-100'>
            ...
            {/* favorite icon can be added here */}
        <TouchableOpacity className='absolute top-2 right-2 
        z-10 p-2 bg-white rounded-full shadow-sm' onPress={(e)=>{
        ...
        {/* is Featured */}
        {product.isFeatured && (
            <View className='absolute top-2 left-2 px-2 
             py-1 bg-black rounded'>
                <Text className='text-white text-xs 
                font-bold uppercase'>Featured</Text>
            </View>
        )}
        </View>
        ...
        <View className='p-3'>
           ...
           text-sm mb-1'numberOfLines={1}>{product.name}
           </Text>
